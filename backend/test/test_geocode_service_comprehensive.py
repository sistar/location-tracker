...
class TestHandler:
    ...
    def test_handler_invalid_coordinates(self):
        ...
        assert response['statusCode'] == 500  # Invalid coordinates cause ValueError exception
        body = json.loads(response['body'])
        assert 'error' in body
    ...
    def test_handler_geocoding_error(self, mock_reverse_geocode):
        ...
        assert response['statusCode'] == 200  # Handler doesn't check for error in geocode result
        body = json.loads(response['body'])
    ...
    def test_handler_coordinate_edge_cases(self, mock_reverse_geocode):
        ...
        edge_cases = [
            ...
            {'lat': '0.1', 'lon': '0.1'},         # Near equator, near prime meridian (not exactly 0)
