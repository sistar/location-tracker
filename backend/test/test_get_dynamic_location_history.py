...
class TestDatabaseOperations:
    ...
    def test_query_location_range_success(self, mock_parse):
        ...
        mock_table.query.return_value = {
            'Items': [
                {'id': 'vehicle_01', 'timestamp': 1681430400, 'lat': 52.5200, 'lon': 13.4050}
            ]
        }
        
        # Mock timestamp parsing to return epoch values
        mock_parse.side_effect = lambda x: datetime.fromtimestamp(x) if isinstance(x, (int, float)) else datetime.fromisoformat(x)
        
        items, error = query_location_range(mock_table, 'vehicle_01', 1681430400, 1681434000)
...
class TestExtendSessionPoints:
    ...
    def test_extend_session_points_backward(self, mock_parse):
        ...
        mock_table.query.return_value = {
            'Items': [
                {'id': 'vehicle_01', 'timestamp': 1681430100, 'lat': 52.5200, 'lon': 13.4050}
            ]
        }
    ...
    def test_extend_session_points_forward(self, mock_parse):
        ...
        mock_table.query.return_value = {
            'Items': [
                {'id': 'vehicle_01', 'timestamp': 1681430700, 'lat': 52.5200, 'lon': 13.4050}
            ]
        }
    ...
    def test_extend_session_points_no_points(self):
        ...
        with patch('handlers.get_dynamic_location_history.parse_timestamp_safely') as mock_parse:
...
class TestHandler:
    ...
    def test_handler_success(self, mock_clean, mock_extend, mock_query):
        ...
        mock_clean.return_value = [
            {'id': 'vehicle_01', 'timestamp': 1681430400, 'lat': 52.5200, 'lon': 13.4050, 'segment_type': 'moving'}
        ]
    ...
    def test_handler_no_query_params(self):
        ...
        with patch('handlers.get_dynamic_location_history.query_location_range') as mock_query:
    ...
    def test_handler_exception(self):
        ...
        with patch('handlers.get_dynamic_location_history.calculate_time_window') as mock_calc:
