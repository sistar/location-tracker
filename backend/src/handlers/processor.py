...
def is_outlier_temporal(location: Dict[str, Any], 
                       threshold_meters: float = 100,
                       max_speed_kmh: float = 150) -> tuple[bool, str]:
    """
    ...
    distance = haversine_distance(
        recent_location["lat"], recent_location["lon"], 
        location["lat"], location["lon"]
    )
    
    print(f"DEBUG: Comparing locations:")
    print(f"  Recent: lat={recent_location['lat']}, lon={recent_location['lon']}, ts={recent_location.get('timestamp', 'N/A')}")
    print(f"  Current: lat={location['lat']}, lon={location['lon']}, ts={location.get('timestamp', 'N/A')}")
        ...
        if curr_timestamp is not None and prev_timestamp is not None:
            ...
            if time_diff_seconds > 0:
                ...
                if speed_kmh > max_speed_kmh:
                    return True, f"Unrealistic speed: {speed_kmh:.1f} km/h (max: {max_speed_kmh} km/h)"
                
                # For very short time gaps, still use distance threshold
                if time_diff_seconds < 10:  # Less than 10 seconds
                    if distance > threshold_meters:
                        return True, f"Large distance in short time: {distance:.1f}m in {time_diff_seconds:.1f}s"
                
                # Speed is reasonable, not an outlier
                return False, f"Reasonable movement: {distance:.1f}m in {time_diff_seconds/60:.1f}min ({speed_kmh:.1f} km/h)"
            elif time_diff_seconds == 0:
                # Same timestamp - use distance only
                if distance > threshold_meters:
                    return True, f"Distance threshold exceeded: {distance:.1f}m (same timestamp)"
            ...
            if distance > threshold_meters:
                return True, f"Distance threshold exceeded: {distance:.1f}m (missing timestamp: {', '.join(missing_ts)})"
            return False, f"Distance within threshold (missing timestamp: {', '.join(missing_ts)})"
            
    except Exception as e:
        # Error processing timestamps, fall back to distance-based detection
        print(f"  ERROR processing timestamps: {e}")
        if distance > threshold_meters:
            return True, f"Distance threshold exceeded: {distance:.1f}m (timestamp error: {e})"
...
def add_to_location_history(location: Dict[str, Any], max_history: int = 10):
    ...
    print(f"DEBUG: Adding to history: lat={location['lat']}, lon={location['lon']}, ts={location.get('timestamp', 'N/A')}")
    ...
    if len(location_history) > max_history:
        ...
        print(f"DEBUG: Removed old location from history: lat={removed['lat']}, lon={removed['lon']}, ts={removed.get('timestamp', 'N/A')}")
    
    print(f"DEBUG: History size after: {len(location_history)}")
    if len(location_history) > 0:
        recent = location_history[-1]
        print(f"DEBUG: Most recent in history: lat={recent['lat']}, lon={recent['lon']}, ts={recent.get('timestamp', 'N/A')}")
...
def process_location(event, context):
    ...
    try:
        ...
        if isinstance(event, dict) and 'body' in event:
            ...
            location_data = {k: v for k, v in request_data.items() if k != 'skip_outlier_detection'}
                
            # Process single location from HTTP request
            result = process_single_location(location_data, skip_outlier_detection=skip_outlier_detection)
...
def process_single_location(location_data, skip_outlier_detection=False):
    ...
    try:
        ...
        if not skip_outlier_detection:
            ...
            if len(location_history) > 0:
                if not is_significant_movement(location_data, location_history[-1], min_distance=3):
